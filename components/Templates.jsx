...
const TemplateItem = React.memo(({ template }) => {
  const handleExternalLinkClick = (e) => {
    ...
    // If there's no deployedLink, redirect to the repositoryLink
    if (!template.deployedLink) {
      window.open(template.repositoryLink, '_blank');
    } else {
      window.open(template.deployedLink, '_blank');
    }
  };
  return (
    <div className="relative overflow-hidden rounded-lg shadow-lg border transition hover:shadow-xl">
      ...
      <div className="absolute bottom-0 left-0 w-full border-t p-3 text-gray-600 dark:text-white text-sm">
        <div className="flex justify-between items-center">
          ...
          <a
            href={template.deployedLink || template.repositoryLink}
